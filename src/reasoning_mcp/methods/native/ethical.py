...
ETHICAL_REASONING_METADATA = MethodMetadata(
    ...
    description="Multi-framework ethical analysis with stakeholder consideration and balanced recommendations",
...
class EthicalReasoning:
    ...
    async def execute(
        ...
        *,
        ...
        consequentialist = self._apply_consequentialist(input_text, stakeholders, context)
        step3 = ThoughtNode(
            id=str(uuid4()),
        ...
        deontological = self._apply_deontological(input_text, stakeholders, context)
        step4 = ThoughtNode(
            id=str(uuid4()),
            type=ThoughtType.BRANCH,
            method_id=self._identifier,
        ...
        virtue = self._apply_virtue_ethics(input_text, stakeholders, context)
        step5 = ThoughtNode(
            id=str(uuid4()),
            type=ThoughtType.BRANCH,
            method_id=self._identifier,
        ...
        care = self._apply_care_ethics(input_text, stakeholders, context)
        step6 = ThoughtNode(
            id=str(uuid4()),
            type=ThoughtType.BRANCH,
            method_id=self._identifier,
        ...
        recommendation = self._synthesize_recommendation(
            input_text, stakeholders, consequentialist, deontological,
            virtue, care, comparison, context
        )
        final = ThoughtNode(
            id=str(uuid4()),
    ...
    def _apply_consequentialist(
        self, scenario: str, stakeholders: list[str], context: dict[str, Any]
    ) -> str:
        ...
        return (
            ...
            f"- May benefit {stakeholders[0] if stakeholders else 'primary stakeholders'} through improved outcomes\n"
            ...
            f"considering both the magnitude and probability of consequences for all stakeholders."
        )

    def _apply_deontological(
        self, scenario: str, stakeholders: list[str], context: dict[str, Any]
    ) -> str:
        ...
        return (
            ...
            f"This framework evaluates based on moral duties, rights, and adherence to rules, "
            f"regardless of consequences.\n\n"
            f"Key rights at stake:\n"
            f"- Individual autonomy and freedom of choice\n"
            f"- Right to be treated with dignity and respect\n"
            f"- Rights of {stakeholders[0] if stakeholders else 'stakeholders'} to protection and fairness\n\n"
            f"Relevant duties:\n"
            ...
            f"Moral principles:\n"
            f"- Kant's categorical imperative: Act only according to principles that could be universal laws\n"
            f"- Respect for persons as ends in themselves, never merely as means\n"
            f"- Justice requires treating like cases alike and respecting fundamental rights\n\n"
    ...
    def _apply_virtue_ethics(
        ...
        """
        return (
            f"**Virtue Ethics Analysis (Character Perspective)**\n\n"
            f"This framework asks what virtues are at stake and what a person of good "
            f"character would do in this situation.\n\n"
            f"Relevant virtues:\n"
            f"- **Wisdom**: Making sound judgments with good information and deliberation\n"
            f"- **Justice**: Giving each person what they are due, treating fairly\n"
            f"- **Courage**: Standing up for what is right despite challenges\n"
            f"- **Temperance**: Exercising moderation and self-control\n"
            f"- **Compassion**: Showing genuine care and empathy for others\n"
            f"- **Integrity**: Consistency between values and actions\n\n"
            f"Character considerations:\n"
            f"- What would a virtuous person do in this situation?\n"
            f"- How does this decision reflect on moral character?\n"
            f"- Does it cultivate or undermine virtue in individuals and community?\n"
            f"- What habits of character does it promote?\n\n"
            f"A virtuous approach seeks the mean between extremes and acts from "
            f"a well-developed moral character rather than just following rules or calculating outcomes."
        )

    def _apply_care_ethics(
        self, scenario: str, stakeholders: list[str], context: dict[str, Any]
    ) -> str:
        ...
        """
        return (
            f"**Care Ethics Analysis (Relational Perspective)**\n\n"
            f"This framework emphasizes relationships, responsibilities of care, "
            f"and the particular context of moral situations.\n\n"
            f"Relational considerations:\n"
            f"- Existing relationships and interdependencies among stakeholders\n"
            f"- Care responsibilities toward vulnerable or dependent parties\n"
            f"- How decisions affect trust and relationship quality\n"
            f"- Power dynamics and asymmetries in relationships\n\n"
            f"Caring responses:\n"
            f"- Attentiveness to the needs and voices of all stakeholders, especially marginalized ones\n"
            f"- Responsiveness to particular contexts rather than abstract rules\n"
            f"- Responsibility for maintaining and nurturing important relationships\n"
            f"- Competence in providing appropriate care\n\n"
            f"Context matters:\n"
            f"- The specific circumstances and history of relationships\n"
            f"- Emotional and psychological dimensions of the situation\n"
            f"- How decisions affect the web of care relationships\n\n"
            f"Care ethics asks us to be attentive to particular needs and relationships "
            f"rather than applying universal rules mechanically."
        )
    ...
    def _compare_frameworks(
        ...
        """
        return (
            f"**Cross-Framework Comparison**\n\n"
            f"After analyzing this scenario through four major ethical frameworks, "
            f"we can identify areas of convergence and tension:\n\n"
            f"**Areas of Agreement:**\n"
            f"- All frameworks recognize the importance of considering stakeholder welfare\n"
            f"- Each emphasizes the need for moral reasoning and justification\n"
            f"- All acknowledge the complexity and significance of the ethical question\n\n"
            f"**Key Tensions:**\n"
            f"- **Consequences vs. Principles**: The consequentialist focus on outcomes may conflict "
            f"with deontological emphasis on absolute rights and duties\n"
            f"- **Universal vs. Particular**: Deontological universal principles may clash with "
            f"care ethics' attention to particular relationships and contexts\n"
            f"- **Action vs. Character**: Virtue ethics asks 'what kind of person should I be?' while "
            f"other frameworks focus more on 'what should I do?'\n\n"
            f"**Complementary Insights:**\n"
            f"- Consequentialist analysis helps us consider practical outcomes\n"
            f"- Deontological analysis protects fundamental rights and maintains moral boundaries\n"
            f"- Virtue ethics reminds us that character and motivation matter\n"
            f"- Care ethics ensures we attend to relationships and particular needs\n\n"
            f"An ethically robust decision should try to honor insights from multiple perspectives."
        )
    ...
    def _synthesize_recommendation(
        ...
        """
        return (
            f"**Balanced Ethical Recommendation**\n\n"
            f"After analyzing this scenario through multiple ethical frameworks, "
            f"here is a synthesized recommendation:\n\n"
            f"**Core Recommendation:**\n"
            f"The most ethically sound approach should:\n"
            f"1. Respect fundamental rights and dignity of all stakeholders (deontological)\n"
            f"2. Consider and weigh consequences carefully, especially for vulnerable parties (consequentialist)\n"
            f"3. Act from virtues like wisdom, justice, and compassion (virtue ethics)\n"
            f"4. Attend to relationships, care responsibilities, and particular contexts (care ethics)\n\n"
            f"**Key Considerations:**\n"
            f"- Ensure meaningful consent and autonomy for {stakeholders[0] if stakeholders else 'all parties'}\n"
